...
def _check_llm(subject: str) -> tuple[bool, float, str] | None:
    ...
    try:
        ...
        prompt = (
            ...
            "Правило: если назначение однозначно сельскохозяйственное или "
            "однозначно нет — confidence 0.85-0.95. Если назначение "
            "неоднозначно, зависит от контекста или требует дополнительных "
            "документов (дроны, ПО, стройка, ремонт, универсальная техника) — "
            "ставь confidence НЕ ВЫШЕ 0.6: такие кейсы уходят оператору.\n"
            "Примеры:\n"
            ...
            'Предмет: "дрон для полей" -> {"matches": true, "confidence": 0.6,'
            ' "reason": "возможное сельхоз-назначение, требует уточнения"}\n'
            f'Предмет: "{subject}"\n'
